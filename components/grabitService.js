...
const EXTENSION_ID = "{77485BDA-6FFE-5A73-B3ED-943F929C15AE}";
const EXTENSION_NAME = "Grabit";
const CHROME_NAME = "grabit";
const SERVICE_NAME = EXTENSION_NAME + " Service";
const SERVICE_CTRID = "@mozilla.org/grabit-service;1";
const SERVICE_ID = "{D535898B-321D-5BBE-9D43-CD525E30AED4}";
...
const INCLUDE = function(name) {
  if (arguments.length > 1)
    ...
    try {
      LOADER.loadSubScript("chrome://grabit/content/"+ name + ".js");
      _INCLUDED[name] = true;
    } catch(e) {
      dump("INCLUDE " + name + ": " + e + "\n");
    }
...
,
  ...
  register: function() {
    ...
    if (mm && mm.addWeakMessageListener) mm.addWeakMessageListener("Grabit::download", this);
  },
  unregister: function() {
    try {
      const os = this.observerService;
      ...
      var mm = this.messageManager;
      if (mm && mm.addWeakMessageListener) mm.removeWeakMessageListener("Grabit::download", this);
    } catch(ex) {
      ...
      switch (name) {
        case "hide-icons":
          ...
          for (var wins = this.windowMediator.getEnumerator(null); wins.hasMoreElements();) {
             w=wins.getNext();
             if (typeof(w.gGrabit)=="object" && w.gGrabit.toggleMainMenuIcon) {
               w.gGrabit.toggleMainMenuIcon();
             }
          }
        break;
...
,
  ...
  hideNativeUI: function(document, selectors) {
    ...
    (document._Grabit_NativeUI_styleSheets ||
      (document._Grabit_NativeUI_styleSheets = [])
    ).push(s);
  },
  restoreNativeUIs: function(document) {
     var ss = document._Grabit_NativeUI_styleSheets;
     if(!ss) return;
     ...
     document._Grabit_NativeUI_styleSheets = null;
  },

  cursor: function(b) {
    DOM.updateStyleSheet('@-moz-document regexp("https?:.*") { body * { cursor: url(chrome://grabit/skin/icon32.png), auto !important }}', b);
  },

  _httpServer: null,
  get httpServer() {
    if (typeof(GrabitHttpServer) != "function") {
      INCLUDE("grabitHttpServer");
    }
    return ((!this._httpServer) || this._httpServer.isDown) ?
       this._httpServer=new GrabitHttpServer(this)
      :this._httpServer;
  }
,
...
,
  showFileWriteError: function(file, e) {
    CC["@mozilla.org/embedcomp/prompt-service;1"].getService(CI.nsIPromptService)
              .alert(null, "Grabit", this.getString("cannotWriteFile", [file.path]));
...
,
  download: function(links, opType, dmName) {
    ...
    if (!dm) {
      this.log("Grabit error: no download manager selected!");
      return false;
    }
...
,
  ...
  logInit: function() {
    ...
    charStream.writeString("*** Grabit started at " + new Date().toISOString() + " ***\n");
...
,
  ...
  get tmpDir() {
    ...
    function prepareTmp(t, create) {
      ...
      t.append("grabit." + encodeURI(fg.profDir.leafName).replace(/%/g,"_"));
      if (t.exists()) {
...
,
  createCustomDM: function(name) {
    ...
    const dm = new GrabitDMCust(name);
    if (name && name.length) {
      GrabitDMCust.persist(this);
      this.sortDMS();
...
,
  sortDMS: function() {
    GrabitDM.dms.sort(function(a,b) {
...
,
  checkDownloadManagers: function(init, detect) {
    ...
    if (init || detect) GrabitDM.init(this);
...
,
  ...
  cleanup: function() {
    ...
    try {
      ...
      try {
        if (typeof GrabitDM === "object") GrabitDM.cleanup(this.uninstalling);
      } catch(eexx) {
        dump(eexx.message);
      }
...
,
  ...
  getMedia: function(w) {
    return "_grabitMedia" in w ? w._grabitMedia : null;
  }
,
