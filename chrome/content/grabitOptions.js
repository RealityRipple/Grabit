...
function fgo_onload() {
  if(!fg) {
    ...
    document.getAnonymousElementByAttribute(document
            .getElementById("grabitOptions"),"dlgtype","cancel")
...
function fgo_showLog() {
  try {
    fg.logWatch();
    open(fg.logURI.spec, "_Grabit_LOG_");
...
function fgo_browseTmpDir() {
  ...
  const title="Grabit - "+$("tmpdir-label").value;
...
function fgo_browseWellGet(reset) {
  ...
  const fp = CC["@mozilla.org/filepicker;1"].createInstance(CI.nsIFilePicker);
  const title="Grabit - "+$("wellget-label").value;
