...
function GrabitGalleryBuilder() {}

GrabitGalleryBuilder.INTERVAL_RX=/\[\s*(\d+)\s*-\s*(\d+)\s*(;{0,1}\s*\d*)\s*\]/;
GrabitGalleryBuilder.INTERVAL_AZ_RX=/\[\s*([a-z]{1})\s*-\s*([a-z]{1})\s*(;{0,1}\s*\d*)\s*\]/i;
GrabitGalleryBuilder.EXPR_RX=/\[\s*([\w]+)\s*\((.*?)\)\s*\]/i;

GrabitGalleryBuilder.prototype = {
  ...
  urlsTableModel: {
    ...
    selectionChanged: function() {
      try {
        gGrabitGB.urlsPreviewDoc.getElementById(
            GrabitGalleryHTML.prototype.galleryId).innerHTML =
  ...
  onload: function() {
    try {
      ...
      if(this.sandbox) {
        ...
        document.getElementById("grabitGB-expr-tab").setAttribute("disabled", "true");
      }
      ...
      this.validateURLs();
      document.getElementById("mainTabs").setAttribute("onselect","gGrabitGB.tabSelected(event)");
    } catch(e) {
      gGrabitService.log(e);
      window.dump(e);
...
,
  get previewTextBox() {
    return document.getElementById("grabitGB-preview-text");
  }
,  
  get contentTextBox() {
    return document.getElementById("grabitGB-content-text");
  }
,
  get referrerTextBox() {
    return document.getElementById("grabitGB-referrer-text");
  }
, 
  get urlsTable() {
    return document.getElementById("grabitGB-urlsTable");
  }
,
  get urlsPreviewDoc() {
    return document.getElementById("grabitGB-urls-preview").contentDocument;
  }
,
  get exprListBox() { 
    return document.getElementById("grabitGB-expr-list");
  }
,
  get exprTextBox() {
    return document.getElementById("grabitGB-expr-text");
  }
,
  trim: function(s) {
...
, 
  checkIntervals: function(url) {
    return url.search(GrabitGalleryBuilder.INTERVAL_RX)>-1 || url.search(GrabitGalleryBuilder.INTERVAL_AZ_RX)>-1;
  }
,
...
,  
  validateURLs: function() {
    ...
    var htmlBuilder = new GrabitGalleryHTML(this);
...
,
  synchronizeIntervals: function(srcBox,dstBox,extFix) {
    ...
    if(dst=="") {
      ...
      var isrc=new GrabitGalleryIterator(this.trim(srcBox.value));
      var idst=new GrabitGalleryIterator(dst);
      dst="";
      var src=""
...
,
  tabSelected: function(ev) {
    switch(ev.target.selectedItem.id) {
      case "grabitGB-url-tab":
        this.exprChanged();
        this.validateURLs();
        break;
      case "grabitGB-expr-tab":
          this.validateURLs();
...
,
  exprSelected: function() {
    ...
    const exprDes=document.getElementById("grabitGB-expr-des");
    const rxFxName=/\bfunction \w+\(/;
    const errorTextBox=document.getElementById("grabitGB-expr-error-text");
...
,
  _hilightErrors: function() {
    ...
    const errTextBox = document.getElementById("grabitGB-expr-error-text");
...
function GrabitGalleryHTML(builder) {
  this.builder=builder;
  this.previews=new GrabitGalleryIterator(builder.previewBase);
  this.contents=new GrabitGalleryIterator(builder.contentBase);
  var exprNames=[];
  var name,expr;
  
  for(var base=builder.previewBase.concat(builder.contentBase), match=null;
    match=base.match(GrabitGalleryBuilder.EXPR_RX);
  ...
  for(name in ee) {
    ...
    this.env[name] = expr.func
      ...
      // (GrabitGalleryBuilder.onload), creating them from the editor text
      // (GrabitGalleryBuilder.createExpression), and calling them
      // (GrabitGalleryHTML.evalExpressions).
...
function GrabitGalleryIterator(base) {
  this.base=base;
  var match=GrabitGalleryBuilder.INTERVAL_RX.exec(this.base);
  var matchAZ=GrabitGalleryBuilder.INTERVAL_AZ_RX.exec(this.base);
  ...
  this.delegate=new GrabitGalleryIterator(
    this.match.input.substring(this.match.index+this.match[0].length)
    );
}
  
GrabitGalleryIterator.prototype = {
  renderedCursor: "",
  reset: function() {
    this.cursor=this.start;
...
var gGrabitGB=new GrabitGalleryBuilder();
