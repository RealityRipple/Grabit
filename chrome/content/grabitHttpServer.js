...
GrabitHttpServer=function(fgService) {
...
,
  onSocketAccepted: function(ss,transport) {
    this.log("Accepted request from "
     ...
     try {
        new GrabitHttpHandler(this,transport);
     } catch(ex) {
       this.log(ex.message);
     }
  }
...
function GrabitHttpHandler(server,transport) {
  this.server=server;
  this.inputBuffer="";
  ...
  buildResponse: function(body,status,contentType) {
    ...
    if(!status) {
      ...
      body="<h1>"+status+"</h1><pre>"
        +body
        +"</pre><h5>Grabit Http Server v. 0.1</h5>"
    }
